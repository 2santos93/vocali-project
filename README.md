...
CI runs formatting first, Prettier for the workspace and `terraform fmt` for the HCL, then `terraform validate` on each root module with `-backend=false`, so it needs no AWS credential and never reads the state. Then linting, type checking, and the unit suites with their coverage thresholds. The Cypress journeys run as a separate job, because they need a built application, a running server and a browser, and a failure there should report on its own line. A dependency audit runs in a third job so that a new upstream advisory cannot mask a genuine test failure, and commit messages are linted on pull requests.

A deploy workflow runs after CI passes on `main`. It assumes the OIDC role, replaces the code of the twelve API functions and the renderer, and syncs the built assets. It deploys code and nothing else: Terraform is still applied by a person, for the reasons in ADR 10. The job declares the `production` environment, which is what makes GitHub mint the `sub` claim the role trusts and what puts the deployment behind an approval; a reviewer and AWS have to agree before anything moves.

Running it found three things that passing tests had not. A dependency audit that had never run reported three advisories in a transitive websocket client. A journey that passed on a laptop failed on a runner, because the click that opens a menu landed before Vue had hydrated and nothing was listening yet. And the renderer had never started at all: `nuxt build` defaults to a preset that serves HTTP and exports no handler, so the function timed out at init, unnoticed because the account's unverified state refuses the URL before a request can reach it. `pnpm --filter @vocali/web build:lambda` is the build that belongs on Lambda, and the archive now reads the bundle and fails the plan when it exports no handler.
