...
describe('ThemeSwitch', () => {
  /*
   * The distinction the whole component turns on. The preference has three
   * values and the switch has two positions, so the position has to come from
   * the palette actually on screen — not from the preference. A reader
   * following a machine set to dark must not be shown a switch that says
   * "off" on a dark page.
   */
  it.each([
    ['system', true, 'true'],
  ...
  /*
   * A switch reports where it was moved to, and moving it is an explicit
   * choice by definition: turning dark off while following a dark machine is
   * asking for light, not asking to keep following.
   */
  it.each([
  ...
  /*
   * Always rendered, never hidden while it applies. A control that disappears
   * once it is the current state is a control nobody finds — and the reader
   * who has to find it is exactly the one who already chose something else.
   */
  it.each([
    ['system', 'true'],
    ['light', 'false'],
