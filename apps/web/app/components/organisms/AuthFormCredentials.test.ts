...
describe('AuthFormCredentials', () => {
  ...
  /*
   * A real `form`, so the Enter key submits from either field. A div with a
   * click handler is a sign-in form that only works with a mouse, which is the
   * one interaction a keyboard user cannot substitute.
   */
  ...
  it('says nothing about empty fields until the form is submitted', () => {
    ...
    // Telling a user their half-typed address is wrong is true and useless.
  ...
  it('carries the autocomplete token the surrounding page asked for', () => {
    ...
    // `current-password` tells it to fill the one it already has. The wrong
    // token turns a manager into an obstacle.
  ...
  it('locks the fields and the button while a request is in flight', () => {
    ...
    // Leaving the button live is how the same sign-in is sent twice.
