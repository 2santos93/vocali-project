...
describe('BaseInput', () => {
  ...
  /*
   * Driven through `find('input')` rather than the component wrapper on
   * purpose. `VueWrapper.setValue` emits `update:modelValue` on the instance
   * itself, so the assertion passes whatever the component does with the
   * event — verified by replacing the emitted value with a constant and
