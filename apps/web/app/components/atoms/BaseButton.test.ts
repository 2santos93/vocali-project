...
describe('BaseButton', () => {
  it('renders its slot content', () => {
    const wrapper = mount(BaseButton, { slots: { default: 'Subir archivo' } });
  ...
  it('stretches to the full width only when asked', () => {
    expect(mount(BaseButton).classes()).toContain('inline-flex');
    expect(mount(BaseButton, { props: { block: true } }).classes()).toContain('w-full');
  });

  it('carries a visible focus ring', () => {
    expect(mount(BaseButton).classes()).toContain('focus-visible:focus-ring');
  });
});
