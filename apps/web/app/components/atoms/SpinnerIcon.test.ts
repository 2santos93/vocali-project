...
describe('SpinnerIcon', () => {
  ...
  it('takes a caller-supplied label', () => {
    const wrapper = mount(SpinnerIcon, { props: { label: 'Transcribiendo' } });
  ...
  it.each([
    ...
    const wrapper = mount(SpinnerIcon, { props: { size } });

    expect(wrapper.classes()).toContain(expectedClass);
  });

  it('defaults to the medium size', () => {
    expect(mount(SpinnerIcon).classes()).toContain('h-4');
  });

  it('animates', () => {
    expect(mount(SpinnerIcon).classes()).toContain('animate-spin');
  });
});
