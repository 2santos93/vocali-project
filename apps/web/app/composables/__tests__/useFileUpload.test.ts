...
function fileOf(name: string, type: string, size: number): File {
  ...
  // jsdom derives size from the content, so the limit is faked, not allocated.
...
function gatewayDouble(): GatewayDouble {
  ...
  const gateway: GatewayDouble = {
    ...
    getTranscription(transcriptionId: string): Promise<Transcription> {
      ...
      // The last record repeats once the script runs out, so a test only has to
      // describe the transitions it cares about.
    ...
    // suite never spends the minutes the real budgets allow.
...
describe('uploadToPresignedPost', () => {
   ...
   * it cannot report how much of a body has been sent.
   */
  ...
  /*
   * The key, not the sentence: which of the three the transport produced is
   * this function's decision, and how each reads is checked where it renders.
   */
  it.each([
...
describe('createUploadRequests', () => {
  ...
  /*
   * Parsed rather than asserted: a type argument checks nothing, letting a
   * malformed response fail as an undefined property inside the upload.
   */
...
describe('useFileUpload', () => {
  ...
  it('closes the socket once the upload it was watching is over', async () => {
    ...
    // The browser would otherwise hold it until API Gateway's two-hour ceiling.
  ...
  it('ignores a pushed record belonging to a different upload', async () => {
    ...
    // Falling through to the fallback is what "ignored" means here: acting on
    // the foreign record would have settled it as completed.
    expect(controller.phase.value).toBe('stillProcessing');
    expect(controller.transcription.value?.id).toBe('t-1');
  });

  /*
   * The reason the fallback exists: a push that silently fails is worse than
   * the polling it replaced, because nothing is left to notice it.
   */
  ...
  /*
   * A loop with no bound is a tab polling all afternoon. The record is not
   * lost when the budget runs out, so this end state is not an error.
   */
  ...
  /*
   * By this point the file is in storage, so surfacing a read failure as an
   * upload error would claim the audio was lost when it was not.
   */
   ...
   * this is the path a file over 20 MB takes once the client check is bypassed.
   */
