...
import type { ThemeControl } from './useThemePreference';
 ...
 *
 * That is not a detail of the double: it is the reason `useState` sits on top
 * of the cookie here at all. Two callers holding two cookie refs would hold
 * two copies of the preference, and only a reloaded page would agree with
 * itself.
 */
...
describe('useThemePreference', () => {
  beforeEach(() => {
    ...
    cookieOptions.clear();
    /*
     * Only the theme's own key, never the whole map.
     *
     * `useSystemPrefersDark` registers its listener once for the life of the
     * module, so clearing the key it writes to would leave a fresh ref that
     * nothing will ever fill again — every case below would then read a
     * machine permanently set to light.
     */
  ...
  /*
   * `system` is the default, and it is a real third value rather than a way of
   * spelling light. A two-way switch cannot express "follow my machine", so a
   * reader whose laptop turns dark at sunset would otherwise have to come back
   * and change this by hand every evening.
   */
  ...
  /*
   * The cookie value reaches the `class` attribute of the root element, and a
   * cookie is set by whoever has the browser. Anything unrecognised falls back
   ...
   * A theme restored by JavaScript after hydration means the first frame of
   * every page load is painted in the wrong one — a white flash on a dark
   * theme, at every navigation, in a room where the lights are off.
   */
  ...
  /*
   * The gap this composable exists to hold open: `system` must stay the
   * *absence* of a class, because the stylesheet's `color-scheme: light dark`
   * is what hands the choice to the operating system. The switch in the
   * account menu, meanwhile, has two positions and has to show the palette
   * actually on screen — a control reading "off" on a dark page is a control
  ...
  /*
   * An explicit choice overrules the operating system. A function that let the
   * machine win here would quietly undo the choice the reader had just made,
  ...
  /*
   * One piece of shared state, not one ref per caller. The toggle in the
   * account menu and the class on `<html>` are two callers, and if they held
   * separate copies only a reloaded page would agree with itself.
   */
  ...
  /*
   * A year, because nothing about a theme expires with a session, and
   * readable by both sides because the server needs it for the first paint.
   * It carries no authority: the worst a forged value can do is show the other
   * palette, which the fallback above already refuses.
   */
