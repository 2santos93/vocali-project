...
describe('session cookie flags', () => {
  /*
   * Every flag on every cookie rather than a sample, because the failure mode
   * is a fifth cookie added later without them.
   */
  ...
  it('does not rewrite the refresh token when only the access token is renewed', () => {
    ...
    // Rewriting it pushes the eight-hour ceiling forward on every request, so
    // a browser left open would never have to sign in again.
