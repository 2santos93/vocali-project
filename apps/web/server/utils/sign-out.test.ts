...
describe('signing out revokes the session at Cognito', () => {
  /*
   * The property acceptance criterion A3 is written against.
   *
   * Cognito cannot revoke an access token, so what `GlobalSignOut` invalidates
   * is the refresh token — everywhere, not only in this browser. Without the
   * call, clearing the cookies leaves that token valid for the rest of its
   * eight hours in whatever copy of it exists elsewhere, and the user has been
   * told they signed out when they did not.
   */
  ...
  it('renews an expired access token first, so the revocation can authenticate', async () => {
    // Cognito refuses `GlobalSignOut` with an expired token. Skipping the
    // renewal would leave the refresh token alive for a user who waited
    // sixteen minutes before pressing the button — the commonest case there is.
...
describe('when Cognito refuses the revocation', () => {
  ...
  it('reports honestly when Cognito could not be reached, and still clears', async () => {
    ...
    // The refresh token is still live wherever a copy exists. Returning null
    // here would be the cookie-only sign-out this whole file exists to avoid.
