...
describe('registration does not enumerate users', () => {
  /*
   * The property acceptance criterion F6 is written against. An anonymous
   * visitor who types an address into the sign-up form must not learn from the
   * answer whether that address already has an account.
   *
   * `null` is how this module says "answer as though it had succeeded", so the
   * route's reply is identical in both cases.
   */
