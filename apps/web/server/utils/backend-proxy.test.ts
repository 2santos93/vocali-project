...
describe('passing the upstream answer through', () => {
  /*
   * The property the screens are built on. Flattening a refusal into 500 would
   * make "this transcription is not ready" and "the service is broken"
   * indistinguishable at exactly the point the difference decides what the
   * user is told.
   */
  ...
  it('returns the upstream body verbatim, not re-serialised', async () => {
    ...
    // An opaque cursor survives being copied. It does not always survive being
    // parsed and rebuilt by something that re-encodes what it does not know.
...
describe('the API not answering', () => {
  ...
  it('gives every outbound call a deadline, and abandons it when that passes', async () => {
    ...
    // until the platform kills it, and the user watches a spinner that never
    // stops instead of reading something they can act on.
...
describe('building the upstream URL', () => {
  ...
  /*
   * Every path the API actually exposes, so a change to this function has to
   * be reconciled against the real surface rather than against one example.
   * The proxy knows none of these routes — it forwards what it is given — and
   * that is exactly why they are worth stating once.
   */
  it.each([
  ...
  /*
   * The path comes from the address bar, so it is attacker-chosen. Without
   * these checks `//evil.example/x` resolves as protocol-relative and the
   * bearer token is delivered to somebody else's server.
   */
  it.each([
