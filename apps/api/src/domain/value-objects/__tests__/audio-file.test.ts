...
describe('AudioFile', () => {
  ...
  it('rejects a traversal sequence in the file name', () => {
    // S3 keys are opaque and do not resolve `..`, so this escapes nothing
    // today. The guarantee is that no AudioFile carries an unaccepted name.
    const result = AudioFile.create({
      ...validInput,
  ...
  it('rejects a ".." sequence carrying no path separator of its own', () => {
    // The traversal case above is also caught by the separator rule, so only
    // this name can show that the `..` rule does anything.
