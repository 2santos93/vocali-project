...
import { buildContainer, getContainer, resetContainer } from '../composition-root.js';
...
describe('composition root', () => {
  ...
  describe('bucket routing', () => {
    ...
    it('signs the provider read against the audio bucket', async () => {
      ...
      expect(result.success).toBe(true);
      // Signed against the wrong bucket this is a 403 the provider reports,
      // minutes later, as a job that could not fetch its data.
...
/**
 * An unrecognised request is refused rather than passed through, so a test that
 * starts making real calls fails instead of reaching Speechmatics.
 */
