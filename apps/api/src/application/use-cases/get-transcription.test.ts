...
describe('GetTranscription', () => {
  ...
  it('exposes exactly the public field set and nothing internal', async () => {
    ...
    expect(result.success).toBe(true);
    if (!result.success) return;
    // Compared against the shared list rather than spot-checking a few
    // absences: this fails both when an internal field leaks in and when a
    // public one is dropped, and it is the same list the history path is held
    // to, so the two endpoints cannot drift.
