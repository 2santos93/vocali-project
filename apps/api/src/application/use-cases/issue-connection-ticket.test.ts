...
describe('IssueConnectionTicket', () => {
  ...
  /*
   * The lifetime is asserted as a literal rather than imported from
   * `constants.ts`, for the reason recorded there: a test that imports the
   * constant it is checking asserts `constant === constant` and stays green
   * through the window being widened to an hour.
   *
   * Thirty seconds after the injected clock, on both the stored expiry and the
   * one the client is told, because a store that expired the ticket on a
   * different schedule from the one the browser was promised produces a
   * connect that fails for no visible reason.
   */
