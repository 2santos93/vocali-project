...
describe('SaveRealtimeTranscription', () => {
  ...
  it('treats a collision on a freshly generated id as an invariant violation, not a caller error', async () => {
    const { useCase, repository } = buildUseCase();
    // Same reasoning as the upload path: the id is minted moments before the
    // write, so a lost conditional write here cannot happen in production and
    // must not be reported as an ordinary failure.
  ...
  describe('with a client session id', () => {
    ...
    it('returns the original transcription when the same session is saved again', async () => {
      ...
      // Indistinguishable from the first call, which is the requirement: a
      // client that retried because it never saw the response has no way to
      // tell, and no second entry appears in the history.
    ...
    it('keeps one user session id from resolving another user record', async () => {
      ...
      // session id — a plausible collision for a short client-generated
      // string — cannot hand one of them the other's transcription.
