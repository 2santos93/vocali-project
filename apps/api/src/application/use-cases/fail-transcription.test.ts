...
describe('FailTranscription', () => {
  ...
  it('fails a still-pending upload directly, covering the orphaned-job case', async () => {
    const { useCase, repository } = buildUseCase();
    // The provider accepted the job but the save that would record
    // PROCESSING and the job id never happened; the record is still
    // PENDING_UPLOAD with no externalJobId when the failure callback lands.
