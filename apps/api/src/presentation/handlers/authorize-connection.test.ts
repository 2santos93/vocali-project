...
import {
  ...
  type ConnectionAuthorizerEvent,
} from './authorize-connection.js';
...
describe('authorizeConnectionHandler', () => {
  ...
  /*
   * The single-use property as the socket experiences it, and the reason this
   * whole design exists.
   *
   * The ticket rides in a query string, which API Gateway writes into its
   * access log verbatim. Anything replayable from that log is a session
   * whoever reads the log can assume. Asserting the first connect is allowed
   * would pin nothing on its own — an authorizer that never spent anything
   * passes that — so the second attempt with the same ticket is the assertion.
   */
   ...
   * presented value would put every guess — and every real ticket that arrived
   * a moment late — into a log with a fourteen-day retention.
   */
