...
import type { WebSocketRequestEvent } from '../websocket/websocket-event.js';
...
describe('handleConnectionOpenedHandler', () => {
   ...
   * its polling fallback instead of leaving it waiting on a push that will
   * never come.
   */
...
describe('handleConnectionClosedHandler', () => {
   ...
   * to the metrics. The entry it failed to delete is collected by its expiry,
   * which is why every entry carries one.
   */
