...
describe('withAuthenticatedUser', () => {
  ...
  /**
   * The rule this layer exists to enforce. If a fallback to any
   * caller-controlled field is ever added, this test is the one that breaks:
   * the request below carries a plausible user id in the path, the query
   * string and the body, and the only correct answer is still 401.
   *
   * A fallback would not look dangerous in a diff — it would look like
   * tolerance for a route that has no authorizer yet — and it would let any
   * caller name any user and read their clinical history.
   */
