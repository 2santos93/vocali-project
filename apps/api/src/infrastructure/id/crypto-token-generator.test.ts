...
describe('CryptoTokenGenerator', () => {
  it('never repeats a token', () => {
    ...
    // A collision here would mean two users could be issued the same
    // connection ticket, and the second to redeem it would be resolved as the
    // first. Not a proof of randomness — nothing cheap is — but it fails
  ...
  it('is URL-safe, because the token travels in a query string', () => {
    ...
    for (const token of Array.from({ length: 100 }, () => generator.generate())) {
      ...
      // the other a path separator once something in the chain decodes it, so
      // a token containing either is a connect that fails intermittently — on
      // roughly the fraction of tokens that happen to contain one.
