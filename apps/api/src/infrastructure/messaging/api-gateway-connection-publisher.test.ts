...
describe('ApiGatewayConnectionPublisher', () => {
  ...
  /*
   * The mirror case, and the one that matters more.
   *
   * A throttle reported as `gone` would have the caller delete a connection
   * that is still open — permanently, since nothing re-registers it until the
   * browser reconnects. So everything that is not a 410 keeps throwing.
   */
