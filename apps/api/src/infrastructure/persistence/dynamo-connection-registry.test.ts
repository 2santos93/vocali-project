...
describe('DynamoConnectionRegistry', () => {
   ...
   * accepted without complaint and read as a date roughly fifty thousand years
   * out, so the item never expires and nothing anywhere reports a problem —
   * the entry for a browser that vanished is published to for ever.
   *
   * The number is asserted as a literal rather than derived from the input,
   * because deriving it with the same `/ 1000` the adapter uses would assert
   * the adapter against itself.
   */
   ...
   * this user's connections. Under the old layout it was load-bearing: without
   * it the query returned every transcription and idempotency claim the user
   * had, and each was then treated as a connection id and published to.
   */
...
/*
 * The grant, from the side that can be run.
 *
 * The three IAM statements this adapter needs — PutItem, Query and DeleteItem —
 * each carry `dynamodb:LeadingKeys: ["CONN#*"]`. Nothing here can evaluate an
 * IAM policy, so what is checked is the other half of the pair: that every key
 * the adapter builds, on every operation it has, falls inside the prefix the
 * grant allows. A key outside it is denied by the table in production, which is
 * an upload that completes and is never announced.
 *
 * The prefix is written as a literal, deliberately, and it is the same literal
 * that appears in `infra/modules/functions/main.tf`. Nothing compares the two —
 * recorded as a known gap in docs/adr/0011 — so importing the constant here
 * would assert `constant === constant` and leave the Terraform unrepresented on
 * both sides.
 */
