...
describe('DynamoConnectionTicketStore', () => {
  /*
   * The stored key is a digest, not the ticket. A ticket is a bearer
   * credential, so a table export holding them in the clear is an export of
   * usable credentials.
   *
   * The digest is computed here with `node:crypto` directly rather than by
   * importing the mapper's builder: importing it would assert the adapter
   * against the same function it uses, and would stay green if that function
   * were changed to return the ticket unchanged.
   */
  ...
  /*
   * The single-use property, at the level where it is actually enforced.
   *
   * One `DeleteItem` with `ReturnValues: 'ALL_OLD'` both spends the ticket and
   * says whose it was. A `GetItem` followed by a `DeleteItem` would read the
   * same and would leave a window in which two `$connect` attempts both see a
   * live ticket and both open a socket — so the absence of any Get on this
