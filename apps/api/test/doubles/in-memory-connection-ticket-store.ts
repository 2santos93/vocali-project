...
export class InMemoryConnectionTicketStore implements ConnectionTicketStore {
  ...
  redeem(input: { ticket: string; now: Date }): Promise<{ userId: string } | null> {
    ...
    // Deleted whether or not it was still valid: an expired ticket presented
    // once is spent, exactly as the adapter's unconditional-delete-then-inspect
    // spends it. Leaving it behind would let a caller retry it until the
    // store's own sweeper happened to run.
