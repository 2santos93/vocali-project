...
import type { TranscriptionPage } from '../../src/domain/ports/transcription-repository.js';
...
describe('InMemoryTranscriptionRepository', () => {
  ...
  it('does not let identifiers containing "#" collide across users', async () => {
    ...
    // A flat `${userId}#${id}` key would map both saves to the same string
    // ("a#b#c"), so the second save would silently overwrite the first and
    // both lookups below would still return a non-null — but wrong — record.
    // Asserting identity, not just existence, is what catches that.
